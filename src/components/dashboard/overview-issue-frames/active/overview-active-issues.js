...
import { faHourglassHalf } from "@fortawesome/fontawesome-free-solid";
...
export default function OverviewActiveIssues() {
  ...
  return (
    <Collapsible
      open={true}
      trigger={
        <Trigger name="Active" icon={faHourglassHalf} iconClass="active-icon" />
      }
    >
      <div className="Overview-issues-frame">
        {ContextMain.activeIssues.length == 0 ? (
          <NoIssuesFound />
        ) : (
          ContextMain.activeIssues.map(issue => <IssueItem issue={issue} />)
        )}
      </div>
    </Collapsible>
  );
