...
export default function SubmitIssueForm(props) {
  ...
  useEffect(() => {
    TeamsApiService.getTeamUserList(
      TokenService._getUserInfo(TokenService.readJwtToken()).teamId
    )
      .then(users => {
        users.every((v, i) =>
          assigneeOptions.push({ value: v.full_name, label: v.full_name })
        );
      })
  ...
  const onSubmit = DATA => {
    ...
    DATA.team_id = TokenService._getUserInfo(TokenService.readJwtToken()).teamId;
    DATA.creator_id = TokenService._getUserId(TokenService.readJwtToken()); 
    DATA.creator_user_name = TokenService._getUserInfo(TokenService.readJwtToken()).full_name;
    ...
    IssueApiService.postIssue(DATA, DATA.team_id)
      .then(res => {
        ...
        //refresh issues
        IssueApiService.getActiveIssues(DATA.team_id).then(issues => {
          ContextMain.setActiveIssues(issues);
        });
        IssueApiService.getIssuesByTeamId(DATA.team_id).then(issues => {
      ...
      {success ? (
        ...
        <React.Fragment>
          <div className="Sif-header">
            ...
            <span className="Sif-close-b" onClick={props.closeSif}>
              X
            </span>
          </div>
          ...
          <div className="Fis-description">
            <label
              className="Fis-description-label"
              htmlFor="Fis-description-i"
            >
              Description:{" "}
            </label>
            <textarea
              className="Fis-description-i"
              name="description"
              required
              placeholder="Steps to reproduce, what you expected to see, and what you saw instead."
              ref={register}
            />
          </div>
          ...
          <div className="Fis-row">
            ...
            <Select
              ref={register({ name: "priority", required: true })}
              options={defaultOptions.priority}
              onChange={e => changePriority(e.value)}
              name="priority"
            />
          </div>
          {errors.priority && (
            <span className="red">{errors.priority.message}</span>
          )}

          <div className="Fis-row">
            <label className="Fis-label">Severity:</label>
            <Select
              ref={register({ name: "severity", required: true })}
              options={defaultOptions.severity}
              onChange={e => changeSeverity(e.value)}
              name="severity"
            />
          </div>
          {errors.severity && (
            <span className="red">{errors.severity.message}</span>
          )}

          <div className="Fis-row">
            <label className="Fis-label">Assignee:</label>
            <Select
              ref={register({ name: "assignee", required: true })}
              options={assigneeOptions}
              onChange={e => changeAssignee(e.value)}
              name="assignee"
            />
          </div>
          {errors.assignee && (
            <span className="red">{errors.assignee.message}</span>
          )}
          ...
          <div className="Sif-footer">
            <button type="submit" className="Sif-submit-b Radial-button">
              Create Issue
            </button>
            <button
              type="button"
              className="Sif-cancel-b"
              onClick={props.closeSif}
            >
              Cancel
            </button>
          </div>
