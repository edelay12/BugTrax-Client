...
export default function NewEventForm({ issueId, showUpdate }) {
  ...
  const onSubmit = DATA => {
    ...
    DATA.user_name = TokenService._getUserInfo(
      TokenService.readJwtToken()
    ).full_name;
    DATA.team_id = TokenService._getUserInfo(
      TokenService.readJwtToken()
    ).teamId;

    EventApiService.postNewEvent(DATA)
      .then(res => {
        EventApiService.getEventsByIssueId(issueId).then(events => {
    ...
    <div className="NeF-container">
      <form className="NeF-form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="NeF-input-container">
          <label className="NeF-summary-label" htmlFor="NeF-summary">
            Update:{" "}
          </label>
          <input
            className="NeF-summary"
            type="text"
            name="change_summary"
            maxLength={75}
            placeholder="Enter a one-line summary of the update."
            ref={register}
          />
          <label className="NeF-description-label" htmlFor="NeF-description">
            Comments:{" "}
          </label>
          <textarea
            className="NeF-description"
            name="change_description"
            required
            placeholder="Please enter update comments here."
            ref={register}
          />
        </div>

        <div className="NeF-status-container">
          <label className="NeF-status-label" htmlFor="NeF-status">
            Status:{" "}
          </label>
